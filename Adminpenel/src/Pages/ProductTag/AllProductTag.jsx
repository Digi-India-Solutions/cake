    ...
    const fetchProductTags = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/api/get-producttag"
  ...
  const deleteProductTag = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(
            `http://localhost:5000/api/delete-producttag/${id}`
          ); // Replace with your actual API endpoint
            ...
            {productTags.length > 0 ? (
              productTags.map((tag) => (
                <tr key={tag._id}>
                  ...
                  <td>
                    <img
                      src={`http://localhost:5000/${tag.image}`}
                      alt={tag.tagHeading}
                  ...
                  <td>
                    {tag.priceRange.map((range, index) => (
                      <div key={index}>
                        ...
                        <img
                          src={`http://localhost:5000/${range.priceRangeImage}`}
